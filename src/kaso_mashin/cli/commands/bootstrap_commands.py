...
class BootstrapCommands(BaseCommands[BootstrapListSchema, BootstrapGetSchema]):
    ...
    def register_commands(self, parser: argparse.ArgumentParser):
        ...
        bootstrap_create_parser = bootstrap_subparser.add_parser(
            name="create", help="Create bootstrap"
        )
        bootstrap_create_parser.add_argument(
            "-n",
            "--name",
            dest="name",
            type=str,
        ...
        bootstrap_modify_parser = bootstrap_subparser.add_parser(
            name="modify", help="Modify bootstrap"
        )
        bootstrap_modify_parser.add_argument(
            "--uid", dest="uid", type=uuid.UUID, help="The bootstrap uid"
        )
        bootstrap_modify_parser.set_defaults(cmd=self.modify)
        bootstrap_remove_parser = bootstrap_subparser.add_parser(
            name="remove", help="Remove a bootstrap"
        )
    ...
    def create(self, args: argparse.Namespace) -> int:
        schema = BootstrapCreateSchema(name=args.name, kind=args.kind, content="<TODO>")
    ...
    def modify(self, args: argparse.Namespace) -> int:
        schema = BootstrapModifySchema(name=args.name, kind=args.kind, content="<TODO>")
        resp = self._api_client(
            uri=f"/api/bootstraps/{args.uid}",
            method="PUT",
            schema=schema,
            expected_status=[200],
