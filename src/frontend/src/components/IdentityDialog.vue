...
const title = computed(() => {
  switch (props.kind) {
    case DialogKind.create:
      return "Create Identity";
    case DialogKind.modify:
      return "Modify Identity";
    case DialogKind.remove:
...
const rules = {
  ...
  min_2: (value: any) => {
    if (currentItem.kind !== IdentityKind.password) {
      return true;
    }
    return (!!value && value.length >= 2) || "Minimum 2 characters";
  },
  has_key: () => {
    if (currentItem.kind !== IdentityKind.pubkey) {
      return true;
    }
    return (
      (currentItem.credential !== undefined &&
        currentItem.credential.length > 0) ||
      "You must upload an SSH public key"
    );
  },
};
...
<template>
  <v-dialog v-model="isOpen" activator="parent" :persistent="true">
    <v-card>
      <v-card-title
        ><span class="text-h5">{{ title }}</span></v-card-title
      >
      <v-card-text>
        <v-form ref="form">
          <v-container>
            ...
            <v-row>
              <v-col cols="6" sm="6" md="6">
                <v-text-field
                  ...
                  :clearable="true"
                  :disabled="
                    currentItem.kind === IdentityKind.pubkey ||
                    kind === DialogKind.remove
                  "
                  label="Password"
                />
              </v-col>
            ...
            <v-row>
              <v-col cols="12" sm="12" md="12">
                <v-textarea
                  ...
                  :rules="[rules.has_key]"
                  :disabled="
                    currentItem.kind === IdentityKind.password ||
                    kind === DialogKind.remove
                  "
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="blue-darken-1" :rounded="true" @click="onCancel"
          >Cancel</v-btn
        >
        <v-btn type="submit" :rounded="true" @click="onAccept">{{
          acceptLabel
        }}</v-btn>
      </v-card-actions>
    </v-card>
